<script>
    ...
    async function submit(data) {
        ...
        let answer = await API("/buy/edit", data);
        if (!answer || !answer.success) {
    ...
    onMount(async () => {
        ...
        if (buy.complete) {
            Alerter.Error("Bir hata oluştu!", "Alım tamamlanmış!", () => {
                goto("/buy/list");
            });
            return;
        }
        ...
        for (let item of buy.entries) {
            items.push({
                open: false,
                stock: {
                    code: item.stock.code,
                    name: item.stock.name,
                    fullname: item.stock.code + " - " + item.stock.name,
                    no: item.stock.no,
                },
                assets: item.assets,
                cost: item.cost,
...
{#if !loading}
    <Container>
        <Form
            ...
            <Input control="no:integer" label="Alım No" value={buy.no} visible={false} />
            <div class="flex flex-col gap-2">
                <div class="flex gap-2">
                    <div class="flex-1">
                        <Input control="doc:text" label="Belge No" value={buy.doc} />
                    </div>
                    <div class="flex-1">
                        <Select control="account_no" label="Satıcı" list={accounts} text={buy.account_name} value={buy.account_no} />
                    </div>
                    <div class="flex-1">
                        <Select control="warehouse_no" label="Depo" list={warehouses} text={buy.warehouse_name} value={buy.warehouse_no} />
                    </div>
                </div>
