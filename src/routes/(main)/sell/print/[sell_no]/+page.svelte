<script>
    ...
    onMount(async () => {
        ...
        if (!sell) {
            Alerter.Error("Bir hata oluştu!", "sell çekilemedi", () => {
                goto("/sell/list");
            });
            return;
        }
        ...
        ">
            <div class="flex-1 print:flex-none">
                <div class="
                    ...
                    print:p-2
                ">
                    <h2 class="text-lg font-semibold print:text-base">{sell.doc}</h2>
                    <div class="print:text-sm">{time(sell.moment)}</div>
                </div>
