<script>
    ...
    onMount(async () => {
        ...
        if (sell.complete) {
            Alerter.Error("Bir hata oluştu!", "Satış tamamlanmış!", () => {
                goto("/sell/list");
            });
            return;
        }
        ...
        warehouse_no = sell.warehouse.no;

        for (let item of sell.entries) {
            items[item.stock.no] = {
                open: false,
                stock: item.stock,
                assets: item.assets,
    ...
    async function edit_sell(data) {
        if (!keys(items).length) {
            Alerter.Info("Uyarı", "En az bir stok eklemelisiniz");
            return;
        }
        let answer = await API("/sell/edit", data);
        if (!answer || !answer.success) {
            Alerter.Error("Satış Düzenleme Başarısız!", answer.message);
            return;
        }
        goto("/sell/list");
    }
...
{#if !loading}
    <Container>
        <Form onsubmit={(data) => edit_sell(data)}>
            <Input control="sell_no:integer" label="Satış No" value={sell.no} visible={false} />
            <div class="flex flex-col gap-2">
                <div class="flex gap-2">
                    <div class="w-full">
                        <Vision label="Depo" text={sell.warehouse.name} />
                    </div>
                </div>
                <div class="w-full">
                    <LiveTrigger
