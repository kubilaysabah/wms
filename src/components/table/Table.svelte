<script>
    ...
    export let selection_items = [];

    export let show_selection = false;
    ...
    let show_selection_element = null;
    let show_selection_attached = false;
    let show_selection_mouseenter = (event) => {
        if (!show_selection_element) {
            return;
        }
        show_selection_element.style.display = "block";
        show_selection_element.style.top = event.pageY - 40 + "px";
        show_selection_element.style.left = event.pageX + 40 + "px";
    };
    let show_selection_mouseleave = (event) => {
        if (!show_selection_element) {
            return;
        }
        show_selection_element.style.display = "none";
    };
    let show_selection_mousemove = (event) => {
        if (!show_selection_element) {
            return;
        }
        show_selection_element.style.top = event.pageY - 40 + "px";
    ...
    $: if (show_selection && table && !show_selection_attached) {
        table.addEventListener("mouseenter", show_selection_mouseenter);
        table.addEventListener("mouseleave", show_selection_mouseleave);
        table.addEventListener("mousemove", show_selection_mousemove);
        show_selection_attached = true;
    }
    ...
    if (select) {
        ...
        for (let i = 0; i < selection.length; i++) {
            selection_control[selection[i]] = selection[i];
        }
    }
    ...
    $: all_selected = (() => {
        ...
        for (let i = 0; i < actual_data.length; i++) {
            if (!(actual_data[i][select] in selection_control)) {
                return false;
            }
        }
        return true;
...
{#if show_selection}
    <div class="fixed z-10 py-1 px-2 rounded-md bg-fore text-back" bind:this={show_selection_element}>
        {#if typeof show_selection === "string"}
            {selection.length} {show_selection}
        {:else}
            {selection.length}
        {/if}
    </div>
...
{:else}
    <div class="w-full relative @container overflow-x-auto animate-fade-in" bind:this={table}>
                                ...
                                onclick={() => {
                                    for (let item of actual_data) {
                                        if (any_selected) {
                                            ...
                                            delete selection_control[item[select]];
                                        } else {
                                            if (!onselect(item, true, true)) {
                                                continue;
                                            }
                                            selection_control[item[select]] = item[select];
                                        }
                                    }
                                    selection_control = { ...selection_control };
                                    selection = Object.keys(selection_control);
                                    selection_items = [];
                                    for (let item of actual_data) {
                                        if (selection_control[item[select]]) {
                        ...
                        {#if select}
                            <td class="bg-float-light @{breakpoint}:table-cell block @{breakpoint}:w-auto w-full @{breakpoint}:text-left text-right p-3 @{breakpoint}:static relative @{breakpoint}:pl-3 before:@{breakpoint}:hidden before:content-[attr(data-header)] before:absolute before:left-3 before:top-3 before:font-bold break-words" data-header="">
                                ...
                                <div
                                    class="flex items-center cursor-pointer w-6 h-6 justify-center border border-border-mid rounded bg-back transition-all duration-200"
                                    onclick={() => {
                                        if (selection_control[item[select]] !== undefined) {
                                    ...
                                    <svg class="w-5 h-5 text-brand {selection_control[item[select]] !== undefined ? 'block' : 'hidden'} pointer-events-none" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3">
                                        <path d="M5 13l4 4L19 7" />
                                    </svg>
                                </div>
                            </td>
